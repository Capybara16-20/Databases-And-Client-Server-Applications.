...
namespace Forms
{
    public partial class EditEmployeeForm : Form
    {
        ...
        public EditEmployeeForm(BusinessLogic bl, int id, string fullName)
        {
            InitializeComponent();
            this.bl = bl;
            this.id = id;
            this.fullName = fullName;
            string[] name = this.fullName.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            textBoxSurname.Text = name[0];
            textBoxName.Text = name[1];
            textBoxPatronymic.Text = name[2];
        ...
        private void buttonEditEmployee_Click(object sender, EventArgs e)
        {
                ...
                && (strSpeciality != string.Empty) && (strExperience != string.Empty))
            {
                ...
                bl.EditEmployee(id, fullName, speciality, experience);
                this.Close();
            }
            else
            {
